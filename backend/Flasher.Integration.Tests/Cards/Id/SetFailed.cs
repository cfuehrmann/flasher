...
public sealed class SetFailed : IDisposable
{
    ...
    public async Task Smoke()
    {
        ...
        var settings = new Dictionary<string, string?>
        {
            ...
            { "Cards:FailedMultiplier", $"{multiplier}" }
        };
        ...
        using JsonDocument postResponseJson = JsonDocument.Parse(postResponseString);
        ...
        using HttpResponseMessage response = await client.PostAsync(
            $"/Cards/{cardId}/SetFailed",
            null
        );

        var fromPostUntilSetFailed = DateTime.Now - postChangeTime;

        Console.WriteLine(
            $"Milliseconds from POST until SetFailed responded: {fromPostUntilSetFailed}"
        );

        var maxWaitingTime = fromPostUntilSetFailed * multiplier;
